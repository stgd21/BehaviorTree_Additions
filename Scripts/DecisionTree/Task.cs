...
public class Grow : Task
{
    ...
    public Grow(float newAmount, GameObject newTarget)
    {
        amountToGrow = newAmount;
        target = newTarget;
    }

    public override bool run()
    {
        target.transform.localScale *= amountToGrow;
        return true;
    }
}

public class Stomp : Task
{
    GameObject target;

    public Stomp(GameObject newTarget)
    {
        target = newTarget;
    }
    ...
    public override bool run()
    {
        target.GetComponent<Rigidbody>().AddForce(new Vector3(0, 5, 0), ForceMode.VelocityChange);
        return true;
    }
}